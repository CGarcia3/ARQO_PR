...
for ((i = 0; i < Nfiles; i = i=i+1)) do
    ...
    for ((j=0; j<iter; j = j+1)){
        val_base=$(./edgeDetectorOpt imgs/${files[$i]} | grep Tiempo | awk '{print $2}')
        val_loop=$(./edgeDetectorLoopOrderOpt imgs/${files[$i]} | grep Tiempo | awk '{print $2}')
        val_pragma=$(./edgeDetectorLoopOrderPragmaOpt imgs/${files[$i]} | grep Tiempo | awk '{print $2}')
